...
export const matchCourses = async (subjectGrades, totalPoints) => {
  try {
    const response = await fetch(`${API_BASE_URL}/api/match-courses`, {
      method: "POST",
      headers: {
...
export const fetchCourses = async (params = {}) => {
  try {
    ...
    const url = `${API_BASE_URL}/api/courses?${queryParams.toString()}`
